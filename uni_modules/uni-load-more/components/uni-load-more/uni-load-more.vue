...
<script>
	...
	export default {
		...
		methods: {
			initAutoObserver() {
				if (!this.auto) return
				if (this.autoObserver) return
				if (typeof uni.createIntersectionObserver !== 'function') return
				try {
					this.autoObserver = uni.createIntersectionObserver(this)
					this.autoObserver.relativeToViewport({ bottom: this.autoDistance }).observe('.uni-load-more', (res) => {
						this.autoVisible = !!(res && res.intersectionRatio > 0)
						if (this.autoVisible) {
							this.tryAutoTrigger()
						}
					})
			...
			tryAutoTrigger() {
				if (!this.auto) return
				if (this.webviewHide) return
				if (!this.autoVisible) return
				if (this.status !== 'more') return
				const now = Date.now()
				if (now - this.autoLastTriggerTime < 800) return
				this.autoLastTriggerTime = now
				this.onClick()
			},
			onClick() {
